...
class memberAvGuildList(commands.Cog):

  def __init__(self, bot):
    self.bot = bot
    self.whiteListedChannel = whiteListedChannel

  @commands.command(aliases=['avls', 'avlg'])
  async def avGuildList(self, ctx):
    ...
    if str(ctx.channel.id) not in self.whiteListedChannel:
      ...
      if ctx.message.content != None:
        ...
        if '@' in ctx.message.content:
          ...
          avList = ctx.message.content[6:].split(', ')
          avId = []
          avName = []
          userList = []
          userAvatarList = []
          ...
          for i in range(len(avId)):
            ...
            if member.guild_avatar != None:
              memberAvatar = member.guild_avatar
            elif member.guild_avatar == None and member.avatar != None:
              memberAvatar = member.avatar
            elif member.guild_avatar == None and member.avatar == None:
              memberAvatar = member.default_avatar
            
            
            userAvatarList.append(memberAvatar)
            userName = user.name
...
def setup(bot):
  bot.add_cog(memberAvGuildList(bot))